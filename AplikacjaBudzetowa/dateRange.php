<?php
	...
	$query = "SELECT user_id, SUM(amount), expense_category_assigned_to_user_id FROM expenses WHERE (date_of_expense >= '$fromDay' AND date_of_expense <= '$toDay') AND user_id='$user_id' GROUP BY  expense_category_assigned_to_user_id ORDER BY SUM(amount) DESC";
                    ...
                    <div class="col-lg-9 register-right">
                        ...
                        <div class="tab-content" id="myTabContent">
							...
							<div class="row registerBalance-form justify-content-md-center">
								...
								<?php
								echo "<div id='name2' class='dateRange col-md-12 my-1'>"."od ".$fromDay." do ".$toDay."</div>";
								?>
								...
								<div id="button" class="col-md-6 mb-4">
									...
									<div class="dropdown">
									  ...
									  <div id="myDropdown" class="dropdown-content">
										...
										<a href="dateRange.php" data-toggle="modal" data-target="#exampleModal">Niestandardowy</a>
									  </div>
													...
													mysqli_report(MYSQLI_REPORT_STRICT);
														...
														try 
														{
															...
															else
															{
																...
																$user_id = $_SESSION['id'];
																if($result = $connection->query("SELECT user_id, SUM(amount), income_category_assigned_to_user_id FROM incomes WHERE (date_of_income >= '$fromDay' AND date_of_income <= '$toDay') AND user_id='$user_id' GROUP BY  income_category_assigned_to_user_id ORDER BY SUM(amount) DESC"))
																{
													...
													mysqli_report(MYSQLI_REPORT_STRICT);
														...
														try 
														{
															...
															else
															{
																//echo "id zalogowanego użytkownika:".$user_id = $_SESSION['id'];
																$user_id = $_SESSION['id'];
																if($result = $connection->query("SELECT user_id, SUM(amount), expense_category_assigned_to_user_id FROM expenses WHERE (date_of_expense >= '$fromDay' AND date_of_expense <= '$toDay') AND user_id='$user_id' GROUP BY  expense_category_assigned_to_user_id ORDER BY SUM(amount) DESC"))
																{
																//biezacy miesiac o tym samym id
																//if($result = $connection->query("SELECT user_id, income_category_assigned_to_user_id, amount FROM incomes WHERE MONTH(date_of_income) = MONTH(CURDATE()) AND YEAR(date_of_income) = YEAR(CURDATE()) AND user_id='$user_id'"))
																//{
